...
class LiveStagingController with ChangeNotifier {
  ...
  Future<void> fetchlivestagingreports() async {
    ...
    try {
          ...
          if (responseData.containsKey('results')) {
            ...
            for (var item in currentPageData) {
              if (item['PHYSICAL_WAREHOUSE']?.toString() ==
                ...
                allData.add(item); // Add to class-level allData
              }
            }
      ...
      // Filter the data and assign to filteredData
  ...
  void searchreqno() {
    ...
    // If both search fields are empty, show all filtered data
    if (searchText.isEmpty && searchPickIdText.isEmpty) {
      ...
      filteredData = allData.where((item) {
        String reqno = item['REQ_ID']?.toString().toLowerCase() ?? '';
        String pickid = item['PICK_ID']?.toString().toLowerCase() ?? '';

        bool matchesReqNo = searchText.isEmpty || reqno.contains(searchText);
        bool matchesPickId =
            searchPickIdText.isEmpty || pickid.contains(searchPickIdText);

        return matchesReqNo && matchesPickId;
      }).where((item) {
        // Apply the original filtering condition
        final scannedQty =
            int.tryParse(item['PICKED_QTY']?.toString() ?? '0') ?? 0;
        final previousTruckQty =
            int.tryParse(item['previous_truck_qty']?.toString() ?? '0') ?? 0;
    ...
    notifyListeners();
    print("Search completed. Found ${filteredData.length} items");
  }
  ...
  //   filteredData = allData.where((item) {
  //     String pickid = item['pickid']?.toString().toLowerCase() ?? '';
  //     return searchText.isEmpty || pickid.contains(searchText);
  //   }).toList();
  //   notifyListeners();
  // }
