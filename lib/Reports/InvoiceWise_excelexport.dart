...
// Export Function
// ==========================
Future<void> InvoiceWiseexportGroupedTruckScanDetails(
...
// Write into the merged cell
        ...
        for (final d in invoiceDispatches) {
          ...
          final values = <dynamic>[
            ...
            customerName,
            d['salesman_no'] ?? '',
            d?['salesman_name']?.toString() ?? '',
            d?['driver_name']?.toString() ?? '',
            d['vehicle_no'] ?? '',
            d?['customer_class_code']?.toString() ?? '',
            d?['remarks']?.toString() ?? '',
            d?['transporter_name']?.toString() ?? '',
...
// Set column 21 width
        sheet.setColWidth(21, 20); // Adjust as needed
...
// Set column 22 width
        sheet.setColWidth(22, 25); // Adjust as needed
        row++;
      }
    }
